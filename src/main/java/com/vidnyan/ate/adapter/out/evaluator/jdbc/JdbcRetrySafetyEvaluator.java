...
public class JdbcRetrySafetyEvaluator implements RuleEvaluator {
    ...
    private void checkMethod(SootMethod entryMethod, JavaView view, List<Violation> violations, RuleDefinition rule) {
        ...
        // JDBC-RETRY-001: We are looking for methods that use JDBC but DO NOT have Retry
        if (hasRetry) {
            return; // If it has retry, it's safe (for this rule)
        }
        ...
        if (usesJdbcSink) {
             violations.add(Violation.builder()
                    .ruleId(rule.id())
                    .message("JDBC/JPA operation missing @Retryable or @Retry")
                    .callChain(callChain)
